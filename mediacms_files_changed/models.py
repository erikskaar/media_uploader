...
class Media(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.uploaded_poster and self.uploaded_poster != self.__original_uploaded_poster:
            ...
            # Example: Assuming the tags are stored in the 'description' field as comma-separated values
        if self.description:
            special_chars = "!@#$%^&*()-+?_=,<>/. "
            # Create a translation table for removing special characters
            remove_chars = str.maketrans('', '', special_chars)

            tag_names = [tag.strip().translate(remove_chars) for tag in self.description.split(',')]

            for tag_name in tag_names:
                # Check if the tag already exists, if not create a new one
                tag, created = Tag.objects.get_or_create(title=tag_name)

                # Associate the tag with the media
                self.tags.add(tag)

        # Save the Media instance again to persist the tag associations
    ...
    if self.media_type in ["audio", "image", "pdf"]:
        ...
        if ret.get("fail"):
            ...
            try:
                ...
                self.media_info = ""
            cmd = ["sh", "-c", "(dd if={} bs=1 count=131072; echo -n $(stat --format=%s {})) | md5sum".format(self.media_file.path, self.media_file.path)]

            result = helpers.run_command(cmd)
            if result.get("out"):
                print(result["out"])
                md5sum = result["out"].split()[0]  # Assuming md5sum is the first part of the output
                self.md5sum = md5sum
            self.size = str(ret.get("file_size"))
        else:
            self.media_type = ""
            self.encoding_status = "fail"
...
def media_file_delete(sender, instance, **kwargs):
    ...
    # Clear tags before deleting the Media instance
    if instance.tags.all():
        instance.tags.clear()  # Clear all tags associated with this instance

        # Optionally, update tag media if needed
        for tag in instance.tags.all():
            tag.update_tag_media()
